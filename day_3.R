...
library(magrittr)
library(dplyr)
path <- function(origin, move) {
  move <- str_split_fixed(move, '', n = 2)
  direction <- move[1]
  amount <- as.numeric(move[2])
  steps <- switch(direction,
                  R = data.frame(x = rep(1, amount), y = 0),
                  L = data.frame(x = rep(-1, amount), y = 0),
                  U = data.frame(x = 0, y = rep(1, amount)),
                  D = data.frame(x = 0, y = rep(-1, amount)))
  steps <- cumsum(steps)
  steps <- mutate(steps, 
                  x = x + origin$x, 
                  y = y  + origin$y)
...
# part 2
path_1 <- mutate(path_1, steps_1 = row_number() - 1) #stupid 1 indexing
path_2 <- mutate(path_2, steps_2 = row_number() - 1)
intersections <- inner_join(path_1, path_2)
intersections %>%
  filter(!x == 0 & !y == 0) %>%
  mutate(distance = steps_1 + steps_2) %>%
  arrange(distance) %>%
  pull(distance) %>%
  .[1]
